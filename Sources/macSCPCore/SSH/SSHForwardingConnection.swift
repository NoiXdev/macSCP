...
extension SSHForwardingConnection {
    ...
    /// Read from the checked-out NIO/NIOSSH/Citadel sources, not measured
    /// against the rig, and covering a transport close only: NIO clears
    /// that flag when the channel closes, before it tells a single handler
    /// (`BaseSocketChannel.close0`: the lifecycle transition comes before
    /// `channelInactive`), so it is already `false` when the child channels
    /// this connection carried are failed. For a connection through a jump
    /// host the session's channel is the jump's own child channel, which
    /// goes inactive the same way before the inner session's children are
    /// failed.
