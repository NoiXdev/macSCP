...
struct ReconnectWiringGuardTests {
    ...
    private static let sanctionedSites: [SanctionedSite] = [
        ...
        SanctionedSite(
            ...
            occurrences: 1,
            reason: "`TunnelManager.liveRunner(for:sessions:knownHosts:keys:secrets:connectTimeout:)` — the App layer's one construction of a port-forwarding runner (port-forwarding plan, Task 6). A forwarding is NOT a tab's connection: it belongs to no window, survives every tab, and so cannot be obtained through `ContentView.connect(in:stored:)`. What the rules above buy is preserved rather than bypassed — TOFU stays a hard stop inside the dial (a MISMATCH is decided before any decider is consulted), the decider handed in is the window's own prompt or `.refusing` for autostart, and a session that cannot carry a forwarding — not SSH, bound to a login set, or dialling through a jump host — is refused by `TunnelCarriers.refusalError(for:)`, which `TunnelConnection.connect` throws and `SessionRowTunnelMenuPlan.build` hides the submenu on, so such a session is never offered a profile in the first place (fix round 1 of the CLI sessions-and-tunnels Task 1 — this reason previously named `StoredSessionConnectionConfig.build(for:secret:)`, which answers two of the three rules and words them as the session-connect path's, not a forwarding's). The secret is resolved through `TunnelSecretSources.chain(for:keys:secrets:)`, which is the APP's chain and not the command line's: the session's own Keychain slot first and a managed key's own passphrase slot second, the same precedence `ContentView.fillForm(_:from:)` applies, with no environment variable and no password command anywhere in it (fix round 1 — this reason previously named `secretSources(for:passwordCommand:)`, which put `MACSCP_PASSWORD` ahead of the Keychain and could not reach a managed key's passphrase at all)."),
        SanctionedSite(
            file: "Sources/MacSCPAppKit/TunnelManager.swift",
