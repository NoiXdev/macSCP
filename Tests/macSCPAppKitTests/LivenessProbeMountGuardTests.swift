...
/// Task 4, fix round 2) — inside `ContentView.splitLayout`, which exists in
/// the tree regardless of which tab is active, and never inside
/// `ContentView.detail`, which SwiftUI mounts only for the active tab.
///
...
/// `LivenessProbePolicy`, `.giveUp` through `teardown(_:)`), but nothing
...
struct LivenessProbeMountGuardTests {
    ...
    /// guards: if either anchor ever stops being unique, this suite's two
    ...
    @Test func scannerFlagsTheRunnerMountedInDetailInstead() throws {
        let source = """
            ...
            var detail: some View {
                Group {
                    LivenessProbeRunner(tab: tab, settingsStore: settingsStore, teardown: teardown)
                }
            }
            """
