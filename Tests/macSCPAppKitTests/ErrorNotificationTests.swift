...
struct ErrorNotificationTests {
    ...
    private func makeContentView(poster: RecordingPoster) -> (
        ...
        let view = ContentView(
            ...
            errorNotifier: Self.notifier(poster))
    ...
    @Test func aTransferFailureNotifiesOnlyAboveTheWatermark() {
        #expect(ErrorNotificationPlan.isNewTransferFailure(failureCount: 0, notifiedThrough: 0) == false)
        #expect(ErrorNotificationPlan.isNewTransferFailure(failureCount: 1, notifiedThrough: 0))
        #expect(ErrorNotificationPlan.isNewTransferFailure(failureCount: 1, notifiedThrough: 1) == false)
        #expect(ErrorNotificationPlan.isNewTransferFailure(failureCount: 4, notifiedThrough: 1))
        #expect(ErrorNotificationPlan.isNewTransferFailure(failureCount: 1, notifiedThrough: 4) == false)
    }
    ...
    @Test func aFailedTransferPostsOnceAndARepeatedCheckDoesNotRepost() async throws {
        ...
        // A second failure is a new event.
        enqueueFailingTransfer(on: tab, into: directory)
