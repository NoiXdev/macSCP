...
# keys into /config/ssh_host_keys — never a P-521 ECDSA key. This
# custom-cont-init.d hook (mounted read-only at /custom-cont-init.d, run as
# root before sshd starts) fills that gap for the ecdsa521 host-key-types
# rig service.
#
...
KEY_FILE=/config/ssh_host_keys/ssh_host_ecdsa521_key
REFERENCE_KEY=/config/ssh_host_keys/ssh_host_rsa_key

if [[ ! -f "${KEY_FILE}" ]]; then
    ssh-keygen -t ecdsa -b 521 -N "" -f "${KEY_FILE}"
