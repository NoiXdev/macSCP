...
| [S3 fährt auf der geteilten URL-Session](superpowers/specs/2026-08-29-backlog-s3-teilt-die-url-session.md) | **Erledigt 2026-08-29.** Der Vorgabewert von `URLSessionHTTPTransport` ist weg — jeder Aufrufer nennt seine Session —, und der S3-Wählvorgang baut sich eine eigene aus `URLSessionConfiguration.ephemeral` und gibt sie in `disconnect()` wieder frei. Der Befund davor: `URLSessionHTTPTransport`s Vorgabe war `URLSession.shared`, und **S3 war der einzige Pfad darauf** — WebDAV und die Update-Prüfung bauen sich eine `ephemeral`. Damit landen Antworten in `~/Library/Caches` und eine dauerhafte Weiterleitung wirkt über Neustarts hinweg. Prozessübergreifend nachgemessen: der zweite Prozess folgte einer gecachten 308 zum *Ziel*-Port, ohne den Ursprung noch einmal zu fragen — und über `URLSession.bytes(for:)`, den Download-Pfad, genauso. |
| [S3 folgt Weiterleitungen ohne Kontrolle](superpowers/specs/2026-08-28-backlog-s3-weiterleitungen.md) | Was die Messung **nicht** entwarnt: die Weiterleitung wird gefolgt statt verweigert (die fremde Origin erfährt Bucket-Pfad und Endpunkt), der handgesetzte `Host` reist mit und ist danach falsch, und Foundation streift den Header auch bei gleicher Origin ab — eine legitime Anbieter-Weiterleitung käme unsigniert an. Die Session trägt weiterhin keinen Delegate — seit 2026-08-29 ist sie allerdings S3s eigene und *könnte* einen tragen, was vorher unter `URLSession.shared` gar nicht ging. **Entschieden: eigener Vorgang.** |
...
| [Verwaltungs-Sheets](superpowers/specs/2026-08-20-backlog-verwaltungs-sheets.md) | **Punkte 3 und 5 erledigt** (Spaltensortierung bei Known Hosts; Drei-Punkte-Menü für Import/Export in Login-Sets und SSH-Schlüsseln, mit der Regel „Auswahl-Aktionen bleiben sichtbar, Datei-Aktionen wandern ins Menü"). Punkt 4 ist verworfen. **Offen: Punkt 2** (Facetten-Schnellfilter unter der Suche in Known-Hosts, Keys und Logins — die Suche gibt es dort, den Facetten-Filter nur im Audit-Log) und **Punkt 1** (Sitzung duplizieren). Nachzug: `SnippetsSheet` zeichnet weiterhin eigene Export/Import-Knöpfe und unterläuft die Regel. |
