...
| [RSA host keys](superpowers/plans/2026-09-02-rsa-host-key-spike.md) | **Open — spike verdict (b), 2026-09-02.** Registering an `rsa-sha2-512` pair through NIOSSH's public registry does reach the KEX offer and negotiates, but the host key can never be verified without a fork change: NIOSSH uses one `publicKeyPrefix` both as the offered algorithm name and as the key-blob type, and RFC 8332 makes those differ (`rsa-sha2-512` vs `ssh-rsa`) — measured as `unknownPublicKey: ssh-rsa`, and with both prefixes registered `invalidHostKeyForKeyExchange: Expected rsa-sha2-512, got ssh-rsa`. Named change for the fork: `hostKeyAlgorithmNames` on `NIOSSHPublicKeyProtocol`, used in the offer and the identity check, blob lookup and K_S untouched. Plan: `2026-09-02-rsa-host-key-fork-change.md`. The gated test on port 2235 turns red the day it works. |
