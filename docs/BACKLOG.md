...
| [Verwaltungs-Sheets](superpowers/specs/2026-08-20-backlog-verwaltungs-sheets.md) | **Punkt 3 erledigt** (Spaltensortierung bei Known Hosts: `KnownHostsSorting`, `Table(sortOrder:)`). **Offen: Punkt 5** (Drei-Punkte-Menü für Import/Export — im Baum gibt es dafür nichts), **Punkt 2** (Facetten-Schnellfilter unter der Suche in Known-Hosts, Keys und Logins — die Suche `SheetSearchField` gibt es dort, den Facetten-Filter nur im Audit-Log) und **Punkt 1** (Sitzung duplizieren). Die zwei getroffenen Entscheidungen gelten weiter: **keine** Tabellen-Umstellung für Logins und Keys, und „Auswahl-Aktionen bleiben sichtbar, Datei-Aktionen wandern ins Menü". |
