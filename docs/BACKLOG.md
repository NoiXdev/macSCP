...
| [Unbounded file closes](superpowers/specs/2026-08-28-backlog-unbounded-file-closes.md) | **Measured 2026-09-02:** `SFTPFile.close()` alone hangs against a frozen peer, 6/6 runs, confirmed for the two counted sites this measurement covered (`readStream`'s and `write`'s cancellation-branch closes); the other **six** counted sites remain unmeasured. Separate finding: a cancelled Task does not interrupt an in-flight SFTP read/write against a frozen peer, 6/6 — a close-only fix will not reach it. `BoundedClose` is the shape for the close fix; the cancellation fix is undesigned. |
