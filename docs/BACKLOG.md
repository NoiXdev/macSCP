...
| [RSA host keys](superpowers/specs/2026-08-31-backlog-ssh-key-formats.md) | **Open, measured 2026-09-02.** A server offering only an RSA host key fails key exchange: NIOSSH's client offers ed25519 and the three ECDSA curves, nothing else, and Citadel registers no RSA host-key algorithm. Needs `rsa-sha2-256/512` host-key verification in the fork (`NoiXdev/swift-nio-ssh`) — the same fork work the RSA file-key entry needs for signing. The gated test on port 2235 turns red the day it works. |
