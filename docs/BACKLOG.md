...
| | Entry | Core |
|---|---|---|
| [Host-key types in the rig](superpowers/plans/2026-09-02-host-key-types-rig.md) | **Next.** Every TOFU test today negotiates ed25519 because the stock rig offers three host-key types at once and the client prefers that one. Plan: one sshd service per type (ed25519, ECDSA P-256/384/521, RSA), `ssh-keyscan` proof of the override, one gated test per type pinning the recorded key type; a type the client cannot negotiate is recorded, not forced. Agent auth is already covered per type; file keys stay ed25519-only until the key-formats work. |
