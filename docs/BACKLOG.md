...
| [Abbau gegen eine eingefrorene Gegenseite](superpowers/specs/2026-08-25-backlog-abbau-bei-eingefrorenem-peer.md) | **Gemessen und behoben 2026-08-28.** `disconnect()` kam nicht zurück — betreten und nie verlassen innerhalb 120 s und 30 s; genau `sftp.close()` hing, `SSHClient.close()` nicht. Mit Frist: 5,05 s und der Tab wird `.lost`. Offen und benannt: ungegatet hält nichts die Verdrahtung — ob sich der unbegrenzte Aufruf strukturell ausschließen lässt, ist eine Entscheidung. |
