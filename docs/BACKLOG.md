...
| [S3 fährt auf der geteilten URL-Session](superpowers/specs/2026-08-29-backlog-s3-teilt-die-url-session.md) | Produktionsbefund aus der Diagnose eines flatternden Tests: `URLSessionHTTPTransport`s Vorgabe ist `URLSession.shared`, und **S3 ist der einzige Pfad darauf** — WebDAV und die Update-Prüfung bauen sich eine `ephemeral`. Damit landen Antworten in `~/Library/Caches` und eine dauerhafte Weiterleitung wirkt über Neustarts hinweg. Gemessen: prozessübergreifend aus dem Cache beantwortet. Die Naht existiert und wird von WebDAV bereits benutzt. |
