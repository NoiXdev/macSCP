...
| **B-3** | [Connecting to S3 without a bucket](superpowers/specs/2026-08-31-backlog-s3-without-bucket.md) | **Done 2026-09-02** (both halves). A visible "Start at the bucket list" toggle; `ListBuckets` on connect with named outcomes; bucket rows open and nothing else — every transfer entry point asks one predicate in both directions; list mode stores no bucket. Measured on the rig with a root key and a scoped key (MinIO filters the list rather than denying it). Left open: `permissionModel` read by no UI (files on S3/WebDAV still offer a permissions editor); Servinga unmeasured. |
