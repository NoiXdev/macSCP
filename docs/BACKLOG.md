...
| | Entry | Core |
|---|---|---|
| [Host-key types in the rig](superpowers/plans/2026-09-02-host-key-types-rig.md) | **Done 2026-09-02.** One sshd per host-key type (ports 2231–2235), `ssh-keyscan` proof in the rig README, one gated test per type pinning the recorded key type, a mismatch hard stop on ECDSA. Measured: ed25519 and all three ECDSA curves green; **a server with only an RSA host key cannot be connected to** (`keyExchangeNegotiationFailure`, the client offers no RSA algorithm). File keys per type were already pinned; agent auth per type came with B-2. Record in the key-formats entry. |
| [RSA host keys](superpowers/specs/2026-08-31-backlog-ssh-key-formats.md) | **Open, measured 2026-09-02.** A server offering only an RSA host key fails key exchange: NIOSSH's client offers ed25519 and the three ECDSA curves, nothing else, and Citadel registers only what a caller passes in `SSHAlgorithms.publicKeyAlgorihtms` — macSCP passes none, and Citadel's one RSA type is `ssh-rsa` over SHA-1. Candidate route, measured the same day: NIOSSH has a public `NIOSSHAlgorithms.register(publicKey:signature:)`, so an `rsa-sha2-512` host-key type verifying PKCS#1 v1.5 over SHA-512 (swift-crypto's `_CryptoExtras`, already a transitive checkout) could be registered from macSCP without touching the fork — except that NIOSSH uses one prefix both for the algorithm it offers and for parsing the key blob, and for RSA those differ (`rsa-sha2-512` negotiated, blob typed `ssh-rsa`); Citadel's own `Insecure.RSA.PublicKey` is `ssh-rsa` and verifies SHA-1 only. A spike decides whether that wrinkle is a fork change or a second registered type. The gated test on port 2235 turns red the day it works. |
