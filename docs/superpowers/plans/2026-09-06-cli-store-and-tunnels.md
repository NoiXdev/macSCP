...
- [ ] **Step 2: Run** → FAIL. **Step 3: Implement.** **Step 4:** filter PASS, `everySubcommandTheBinaryOffersIsDrivenByACase` will now be RED (the binary offers `tunnels`, nothing drives it) — that red is expected and is closed by Task 6; record it in the report and do NOT add a fake drive.
