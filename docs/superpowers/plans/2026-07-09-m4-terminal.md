...
- [ ] **Step 2: RemoteShell-Protokoll anlegen**
...
- [ ] **Step 3: Fehlschlagender Test — Cancellation am Host-Key-Prompt**
...
- [ ] **Step 5: Cancellation-Handler implementieren**
...
- [ ] **Step 1: Gated fehlschlagende Tests schreiben**
...
- [ ] **Step 2: Rot** — Compile-Fehler (`TerminalPanelViewModel` unbekannt).
...
- [ ] **Step 4: Grün** — `swift test --filter TerminalPanelViewModel` (8 PASS), dann Gesamtsuite.

- [ ] **Step 5: Commit** — `feat: add terminal panel view model` (mit Footer).
...
- [ ] **Step 2: SSHTerminalView implementieren**
...
- [ ] **Step 3: Grün** — `swift build && swift test` (Basis unverändert; App-Target kompiliert mit SwiftTerm).

- [ ] **Step 4: Commit** — `feat: add swiftterm view with deep sea and phosphor styling` (mit Footer).
...
- [ ] **Step 1: BrowserSession + startSession erweitern**
...
- [ ] **Step 2: Terminal-Toggle in der Kopfzeile**
...
- [ ] **Step 3: Panel ins Layout (zwischen Panes und TransferBar)**
...
- [ ] **Step 4: Lebenszyklus — Shell VOR dem Disconnect schließen**
...
- [ ] **Step 5: Grün + Headless-Launch** — `swift build && swift test`; App kurz headless starten (Muster aus M2/M3: Binary in den Bundle-Wrapper kopieren, starten, beenden) — kein Crash beim Startbildschirm.

- [ ] **Step 6: Commit** — `feat: integrate collapsible terminal panel into the browser window` (mit Footer).

---

### Task 5: Abschluss-Verifikation

- [ ] **Step 1:** `swift test` — Gesamtsuite grün (Erwartung: 137 gesamt = 129 nach T0 + 5 gated T1 + 8 T2 — gezählte Skips inklusive; tatsächliche Zahl im Report festhalten).
- [ ] **Step 2:** Rig hoch (HAUPT-Checkout), `MACSCP_ITEST=1` (15 = 10 alte + 5 Shell), `MACSCP_KEYCHAIN=1` 2/2, Rig runter.
- [ ] **Step 3: Visueller Smoke-Test** (Koordinator, Rig läuft): Verbinden mit 127.0.0.1:2222 testuser/testpass → ⌘T → Terminal erscheint mit Tiefsee-Grund + Phosphor-Text; `ls -la` + `echo test` tippen → Ausgabe erscheint; WÄHREND die Shell offen ist im Remote-Pane navigieren (SFTP + Shell gleichzeitig über eine Verbindung); Fenster resizen (window-change); `exit` tippen → „Shell beendet." + „Neu öffnen"-Button, Klick öffnet neue Shell; ⌘T aus/ein; „Trennen" → sauber zurück zum Formular, kein Hänger.
- [ ] **Step 4:** Checkboxen, Commit `docs: mark M4 plan tasks as completed` (mit Footer).
