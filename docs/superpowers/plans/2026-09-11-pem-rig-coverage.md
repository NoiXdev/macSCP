...
- [x] **Step 1: Failing cells** in `FileKeyTypeIntegrationTests` (same suite, same `MACSCP_ITEST` gate, same connect-and-list body as `fileKeyAuthenticatesThroughMacSCP`, factored into a private `expectLogin(keyPath:passphrase:)` helper used by every cell in the file — recount the "cells" MARK comment):
...
- [x] **Step 2: Run red.** `MACSCP_ITEST=1 swift test --filter FileKeyTypeIntegrationTests` with the rig up: cells 1-3 red BEFORE `installAuthorizedKey` exists (compile) and, once it compiles, red when the install call is commented out for one run (the rig refuses the key — record the error); cell 4's refusal half is green by construction and its conversion half red when `copyAsOpenSSH` is skipped (record). Record every first failing line.
- [x] **Step 3: Implement** the two helpers and un-comment the installs.
- [x] **Step 4: Run green** twice (`MACSCP_ITEST=1 … --filter FileKeyTypeIntegrationTests`), then the whole unit suite (`swift test`) green, `swift build --build-tests` zero warnings. Record the cell counts (existing 10 + PEM 6 + new: 2 + 1 + 1 + 2 = 6 → 22 cells plus Step 0).
- [x] **Step 5: Commit** `test(ssh): every readable PEM variant logs in to the rig, and a DES-EDE3 key is refused then converted`.

Implemented with `pbes2PEM(pkcs8DER:passphrase:rounds:declaredRounds:)` — the plan's `prf:`/`cipherKeyLength:` parameters had no caller that varies them; accepted at review.
