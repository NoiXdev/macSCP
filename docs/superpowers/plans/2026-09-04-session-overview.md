...
- [ ] **Step 1: Red first** — the guard file with its scans against the tree: red (no `SessionOverviewView`).
- [ ] **Step 2: Implement** view, wiring, catalogs; `swift test` green; zero warnings; the existing sidebar guards (`SessionSidebarErrorGuardTests`, `TabContextMenuWiringGuardTests`, `ConnectionFormScrollGuardTests`) still green.
- [ ] **Step 3: Commit** `feat(sidebar): a click shows the session's overview — facts, history, snippets, three actions`.
...
- [ ] **Step 1: Red first** — the sequence test against a stub connect that fails → the fake terminal received nothing; against one that succeeds → exactly one send.
- [ ] **Step 2: Implement**; green; zero warnings.
- [ ] **Step 3: Commit** `feat(snippets): Run from the session overview connects, opens the terminal and sends`.
...
- [ ] **Step 1:** the row and the sentence; count the guard's checks and the model tests.
- [ ] **Step 2: Commit** `docs: the session overview is in`.
