...
- [x] **Step 1: Red first** — the nil-metadata assertion and the symlinked-parent path assertion.
- [x] **Step 2: Implement**; `swift test --filter LocalFileSystem` green; full `swift test`; zero warnings.
- [x] **Step 3: Commit** `feat(local): the listing returns names and kinds from the directory read alone`.
...
- [x] **Step 1: Red first** — `cannot find 'LocalMetadataSource'`; the parked-entry test.
- [x] **Step 2: Implement**; green; zero warnings.
- [x] **Step 3: Commit** `feat(local): metadata arrives per entry as a stream, and a stuck entry is left behind`.
...
- [x] **Step 1: Red first** — the merge tests; the guard.
- [x] **Step 2: Implement**; `swift test` green; zero warnings.
- [x] **Step 3: Commit** `feat(browser): the local pane shows the folder first and fills the details as they arrive`.
