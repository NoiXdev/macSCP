...
- Test: `Tests/macSCPCoreTests/S3FileSystemTests.swift` (mock transport: the three outcomes; root-mode path resolution table: `/` → list, `/b` → `(b, "")`, `/b/x/y` → `(b, "x/y")`, a path without a bucket component in list mode is an error, not a request to `/`), `Tests/macSCPCoreTests/S3IntegrationTests.swift` (gated: root key + toggle → two bucket rows named `macscp-seed`, `macscp-second`, both directories, `modifiedAt` non-nil; open `macscp-seed` → `a.txt`, `sub`; scoped key + toggle → **measured on 2026-09-02 against this MinIO release: NOT `bucketListForbidden` — MinIO returns the filtered one-bucket list** (`macscp-seed` only; an explicit `Deny` on `s3:ListAllMyBuckets` does not change that), so the gated assertion is "one bucket row named `macscp-seed`, and opening `macscp-second` is `AccessDenied`"; the 403 → `bucketListForbidden` path (what AWS's documentation says it does for a key without `s3:ListAllMyBuckets` — unmeasured here, no AWS key in the rig) is pinned with the mock transport, not the rig; scoped key + bucket `macscp-seed`, toggle off → today's connect works — the "usual case" the entry names)
